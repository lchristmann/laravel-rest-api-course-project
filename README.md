...
The project uses the below [Laravel Docker Examples Project](https://docs.docker.com/guides/frameworks/laravel/development-setup/) as starter.

Below are the three access tokens that I locally generated by calling the `/setup` web route.
I got that database content persisted via a Docker volume.
If you're on a new machine call the `/setup` route yourself.
If you or I want to create new tokens, `DELETE FROM personal_access_tokens` and call the route again. 

```json
{
    "admin":"1|juHy9s9bTX1fxtC4Mc6Fh8kFASXaSTBAzzt8n2DM068397ec",
    "update":"2|j2HyAr7XUhNYailBZTOpqtBk7sh8QamUWVtaDklc654aafa4",
    "basic":"3|Vfb1JNxonBiWX2I1a0Ngy3Co0cTZdx3g7ZZn2qIxfd481953"
}
```

Helpful commands: (on top of the ones further down in this README)

```shell
docker compose -f compose.dev.yaml up -d
docker compose -f compose.dev.yaml down
docker compose -f compose.dev.yaml exec postgres bash
  psql -d app -U laravel # password is "secret"
  \dt
  \d customers
  SELECT * FROM personal_access_tokens;
docker compose -f compose.dev.yaml exec workspace bash
  php artisan tinker
  Customer::factory()->make()->toJson()
  Invoice::factory()->count(2)->make()->toJson()
```
